...
REPO_URL = 'https://github.com/miracode/django_tdd.git'
env["user"] = "ubuntu"
...
def _update_static_files(source_folder):
    run('cd %s && ../virtualenv/bin/python2.7 manage.py collectstatic --noinput' % (
        source_folder, ))


def _update_database(source_folder):
    run('cd %s && ../virtualenv/bin/python2.7 manage.py migrate --noinput' % (
        source_folder, ))
